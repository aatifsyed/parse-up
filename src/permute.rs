...
trait ContextlessPermuteParserSequence<'input, Out> {
    fn contextless_permute(&self, input: &'input str) -> ContextlessUpResult<'input, Out>;
}
trait ContextualPermuteParserSequence<'input, Out, Ctx> {
    fn contextual_permute(&self, input: &'input str, ctx: Ctx) -> UpResult<'input, Out, Ctx>;
}

impl<'input, Out, ParserSequence> ContextlessUpParser<'input, Out> for Permute<ParserSequence>
where
...
impl<'input, Out0, Parser0> ContextlessPermuteParserSequence<'input, (Out0,)> for (Parser0,)
where
    Parser0: ContextlessUpParser<'input, Out0>,
{
    fn contextless_permute(&self, input: &'input str) -> ContextlessUpResult<'input, (Out0,)> {
        one_of((series((self.0.borrowed(),)).map_yes(|(yes0,)| (yes0,)),)).parse_contextless(input)
    }
}

impl<'input, Out0, Out1, Parser0, Parser1> ContextlessPermuteParserSequence<'input, (Out0, Out1)>
    for (Parser0, Parser1)
where
    Parser0: ContextlessUpParser<'input, Out0>,
    Parser1: ContextlessUpParser<'input, Out1>,
{
    fn contextless_permute(&self, input: &'input str) -> ContextlessUpResult<'input, (Out0, Out1)> {
        one_of((
...
impl<'input, Out0, Out1, Out2, Parser0, Parser1, Parser2>
    ContextlessPermuteParserSequence<'input, (Out0, Out1, Out2)> for (Parser0, Parser1, Parser2)
where
    Parser0: ContextlessUpParser<'input, Out0>,
    Parser1: ContextlessUpParser<'input, Out1>,
...
mod tests {
    ...
    fn contextless() {
        ...
        assert_eq!(
            ...
            Err(go_on(["yes", "no", "maybe"]).closed().no_ctx()),
        );

        assert_eq!(
            parser.parse_contextless("yes"),
            Err(go_on(["no", "maybe"]).closed().no_ctx()),
        );
        assert_eq!(
            parser.parse_contextless("no"),
            Err(go_on(["yes", "maybe"]).closed().no_ctx()),
        );
        assert_eq!(
            parser.parse_contextless("maybe"),
            Err(go_on(["yes", "no"]).closed().no_ctx()),
        );

        assert_eq!(
            parser.parse_contextless("yesno"),
            Err(go_on(["maybe"]).closed().no_ctx()),
        );
        assert_eq!(
            parser.parse_contextless("yesmaybe"),
            Err(go_on(["no"]).closed().no_ctx()),
        );
